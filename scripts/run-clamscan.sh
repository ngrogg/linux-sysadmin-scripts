...
local target_dir=("/bin" "/lib" "/opt" "/sbin" "/tmp" "/etc" "/usr" "/var" "/home")
...
echo "$(date +%Y%m%d) Beginning scan on ${target_dir[*]}" | tee -a "$log_file"

# For loop to scan dirs
for directory in "${target_dir[@]}"; do
        echo "Scanning $directory" | tee -a "$log_file"
        ...
        if [[ $? -eq 0 ]]; then
              echo "Scan of $directory completed successfully." | tee -a "$log_file"
        else
              echo "Scan of $directory returned non-zero message, check manually" | tee -a "$log_file"
        fi
...
# Bookend output
echo "$(date +%Y%m%d) Finishing scan on ${target_dir[*]}" | tee -a "$log_file"
