...
# ------------------------------------------------------------------------------
# Script: apacheErrorChecker.pl
# Description: Parses Apache error logs across RPM and DEB-based Linux systems,
...
# Define an array of standard Apache log paths with wildcards.
# This ensures the script works out-of-the-box on different Linux distributions
# and catches logs for multiple virtual hosts.
my @log_patterns = (
    '/var/log/httpd/*error_log',   # RPM-based (Rocky, Alma, CentOS, RHEL)
    '/var/log/apache2/*error.log', # DEB-based standard (Ubuntu, Debian)
    '/var/log/apache2/*error_log'  # DEB-based fallback for custom configs
);
...
if (!@found_logs) {
    die "CRITICAL: No Apache error logs found in standard httpd or apache2 directories.\n";
}
...
foreach my $log_file (@found_logs) {
    ...
    while (my $line = <$fh>) {
        ...
        # (?:[^\]:]+:)?     : Non-capturing group to optionally match module names
        #                     like 'core:' or 'php7:' inside the brackets
        # (?:error|crit|warn|emerg) : Match any of these severe log levels
        # \]                : Match a literal closing bracket
        # /i                : Make the match case-insensitive
        # ----------------------------------------------------------------------
        if ($line =~ /\[(?:[^\]:]+:)?(?:error|crit|warn|emerg)\]/i) {

            # ------------------------------------------------------------------
            # REGEX EXPLANATION 2: Stripping Metadata
            # ------------------------------------------------------------------
            # Apache errors start with several bracketed blocks:
            # [Timestamp] [Module:Severity] [PID] [Client IP]
            # We want to strip all of these off the front so we are just left
            # with the actual error message.
            #
            # ^             : Start of the string
            # (\[.*?\]\s*)+ : One or more occurrences of: an opening bracket,
            #                 anything inside (non-greedy), a closing bracket,
            ...
            # We strip out the specific IP addresses and AH codes.
            # ------------------------------------------------------------------
            # Replace IPs with a generic placeholder
            $line =~ s/client\s+\d{1,3}(?:\.\d{1,3}){3}(?::\d+)?/client [IP_REDACTED]/g;

            # Strip Apache standard AHxxxxx error codes (e.g., AH01071)
            $line =~ s/AH\d{5}:\s*//g;
...
print "\n";
print "=" x 80 . "\n";
printf " TOP %d APACHE WEB SERVER ERRORS\n", $top_n;
print "-" x 80 . "\n";
